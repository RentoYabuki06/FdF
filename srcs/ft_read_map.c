...
/*   Updated: 2024/09/19 16:25:40 by yabukirento      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_get_width(char *filename)
{
	...
	fd = open(filename, O_RDONLY);
	if (fd == -1)
		ft_error_and_exit("failed open. (at get_width)\n");
	line = get_next_line(fd);
	...
	if (close(fd) == -1)
		ft_error_and_exit("failed close. (at get_width)\n");
	return (width);
}
...
void	ft_read_map(char **filename, t_map *map)
{
	...
	fd = open(*filename, O_RDONLY);
	if (fd == -1)
		ft_error_and_exit("failed open. (at read_map)\n");
	map->point3d = malloc(sizeof(t_point) * map->height * map->width);
	if (map->point3d == NULL)
		ft_error_and_exit("failed malloc. (at read_map)\n");
	while (true)
	{
		line = get_next_line(fd);
		if (line == NULL)
	...
	if (close(fd) == -1)
		ft_error_and_exit("failed close. (at read_map)\n");
}
